...
![preview](./需要上交的资料//_2025-May-29_01-58-35AM-000_CustomizedView16397410250.jpg)
...
## 数学与算法（简要)
> 参考了中科大Zhanchi Wang团队24年发表在cell上的论文，见参考与致谢部分
...
## 目录结构（重要文件）
- `./设计&建模文件/line.py`：2D 分段与 DXF 导出脚本（含可视化）。
...
- `./设计&建模文件/2D触手 v6.f3z`：2D触手的Fusion360建模模型文件
- `./设计&建模文件/3D鞭子(v.2) v3`:3D触手的Fusion360建模模型文件
- `./设计&建模文件/line.dxf` / `./设计&建模文件/line_*.dxf`：DXF 输出（运行后生成）。
- `./打印文件/*`：现成的 .3mf 打印文件集合（各零部件）。
- `./公式.md`：数学推导与实现说明。
- `./LICENSE`：许可协议。
...
## 参考与致谢
- [1] Z. Wang, N. M. Freris和X. Wei, 《SpiRobs: Logarithmic spiral-shaped robots for versatile grasping across scales》, Device, 卷 3, 期 4, 4月 2025, doi: 10.1016/j.device.2024.100646.
- SciPy（Brent 法求根）
- NumPy / Matplotlib
- ezdxf
